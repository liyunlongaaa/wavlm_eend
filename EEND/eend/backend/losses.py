...
def vad_loss(ys: torch.Tensor, ts: torch.Tensor) -> torch.Tensor:
    ...
    vad_ts = (torch.sum((ts != -1)*ts, 2, keepdim=True) > 0).float()   #有大于0表示说话， （B, T, 1）
    ...
    silence_prob = torch.prod(ys_silence_probs, 2, keepdim=True)  #返回指定维度上的乘积
    ...
    loss[torch.where(torch.max(ts, 2, keepdim=True)[0] < 0)] = 0 #padding的loss记为0，不算
    # normalize by sequence length
