...
def parse_arguments() -> SimpleNamespace:
    ...
    parser.add_argument('--upstream_trainable', default=False, type=bool)
...
if __name__ == '__main__':
    ...
    model = average_checkpoints(    # args.epochs个模型权重平均, 这个会导致预训练模型输出为空字典
    ...
    for i, batch in enumerate(tqdm.tqdm(infer_loader)):
        ...
        with torch.no_grad():
            y_pred = model.estimate_sequential(features, args)[0]
        post_y = postprocess_output(
            y_pred.cpu(), args.subsampling,
