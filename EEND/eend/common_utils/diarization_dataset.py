...
class KaldiDiarizationDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        for rec in self.data.wavs:
            data_len = int(
                self.data.reco2dur[rec] * sampling_rate / frame_shift)   #幀數
    ...
    def __getitem__(self, i: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        Y, T = features.get_labeled_speech(
            self.data,
            rec,
            st,
            ed,
        ...
        # Y_spliced = features.splice(Y, self.context_size)    #拼接，单个帧的信息可能不够充分，因此需要利用周围多帧的信息来增强特征表示能力。
        ...
        return torch.FloatTensor(Y), torch.FloatTensor(T), rec   #(T, dim)  (T, c)
